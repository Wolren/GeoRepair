    ...
    for i in 0..n {
        for j in (i + 2)..n {
            ...
            let ti = ((edges[j].start.x - edges[i].start.x) * (edges[j].end.y - edges[j].start.y)
                - (edges[j].start.y - edges[i].start.y) * (edges[j].end.x - edges[j].start.x))
                / denom;
            let tj = ((edges[j].start.x - edges[i].start.x) * (edges[i].end.y - edges[i].start.y)
                - (edges[j].start.y - edges[i].start.y) * (edges[i].end.x - edges[i].start.x))
                / denom;

            if ti >= -eps && ti <= 1.0 + eps && tj >= -eps && tj <= 1.0 + eps {
                let pi = Coord {
                    x: edges[i].start.x + ti * (edges[i].end.x - edges[i].start.x),
                    y: edges[i].start.y + ti * (edges[i].end.y - edges[i].start.y),
                };
                let pj = Coord {
                    x: edges[j].start.x + tj * (edges[j].end.x - edges[j].start.x),
                    y: edges[j].start.y + tj * (edges[j].end.y - edges[j].start.y),
                };
